...
The same object mapper used when converting from <<from-js-to-java>> deserializes the return values in Java to the corresponding json objects before sending them to client-side.
