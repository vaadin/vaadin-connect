...
When calling a Java service from JavaScript code, the `ConnectClient` will send parameters in JSON format to Java backend side, the parameter will be converted to Java types using the default https://github.com/FasterXML/jackson[ObjectMapper from Jackson Project]. You can also use your own ObjectMapper by providing an ObjectMapper bean with the qualifier defined in `com.vaadin.connect.VaadinConnectController#VAADIN_SERVICE_MAPPER_BEAN_QUALIFIER`.
