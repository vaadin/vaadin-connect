...
When started, Vaadin Connect processes the backend code metadata and automatically exposes the urls for calling the services.

After this, it is possible to call any service method via POST request.
...
Note that Vaadin Connect does not take into account the case of the service name and method, so the url above and
`http://localhost:8080/connect/CounterService/addOne` or `http://localhost:8080/connect/COUNTERSERVICE/ADDONE`
or any other case combination for the service and method name point to the same service method.
