...
public class OpenApiJavaParserImpl implements OpenApiGenerator {
  ...
  private void init() {
    ...
    try {
      ...
      throw new IllegalStateException(
        "There is an error while parsing java source");
    }
  ...
  private boolean isBeanType(Type type) {
    ...
    boolean isPredefinedType =
      NUMBER_TYPES.contains(typeName) || STRING_TYPES.contains(typeName) ||
        BOOLEAN_TYPES.contains(typeName) || MAP_TYPES.contains(typeName) ||
        COLLECTION_TYPES.contains(typeName);
