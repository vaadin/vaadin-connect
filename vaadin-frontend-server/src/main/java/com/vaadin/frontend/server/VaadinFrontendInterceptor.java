...
public class VaadinFrontendInterceptor
  ...
  // We don't use HandlerInterceptor.postHandle here because response comes
  // modified and that fact prevents forwarding
  @Override
  public boolean preHandle(HttpServletRequest request,
    ...
    if (o instanceof ResourceHttpRequestHandler) {
      ...
      // modifies the response, we ResourceHttpRequestHandler.getResource in
      // order to guarantee to do the same check that handleRequest does.
      Method getResource = o.getClass().getDeclaredMethod("getResource",
          HttpServletRequest.class);
      getResource.setAccessible(true);

      // Check whether the static resource exists
      if (getResource.invoke(o, request) == null) {

        // Do the forward and stop spring handler for static content
